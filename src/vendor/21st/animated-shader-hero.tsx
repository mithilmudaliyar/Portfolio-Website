/*
 * RAW, UNMODIFIED pull from 21st.dev (ravikatiyar162/animated-shader-hero).
 * Not wired into the app. Reference only — see /goal for adaptation plan.
 *
 * DO NOT mount this as a replacement Hero — it duplicates the existing
 * hand-built Hero/Constellation. The valuable part is the pointer-reactive
 * fbm-cloud fragment shader inside useShaderBackground (credit: Matthias
 * Hurrle / @atzedent) — mine that GLSL into the shared Atmosphere pass.
 *
 * Also note: uses `<style jsx>` (Next.js styled-jsx) which does NOT work
 * in a plain Vite project without an extra plugin — strip it entirely if
 * adapting any of the surrounding JSX, and replace with plain CSS. All
 * Tailwind classes here are inert too since this project has no Tailwind.
 * Source: https://21st.dev/@ravikatiyar/components/animated-shader-hero
 */
